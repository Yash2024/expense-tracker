...
@Component({
  selector: 'app-jul',
  templateUrl: './jul.component.html',
  styleUrls: ['./jul.component.css']
})
export class JulComponent implements OnInit {
  ...
  constructor() {
    this.localitem = localStorage.getItem("julexplist");
  ...
  saveexp(exp: expense, i: number) {
    if (exp.amount === 0 || exp.amount === null) {
      ...
      localStorage.setItem("julexplist", JSON.stringify(this.explist));
      this.calctot();
    }
  }
  ...
  addexpense() {
    ...
    if (expd.amount === 0 || expd.amount === null) {
      ...
      expd.expid = 7000 + this.explist.length;
      expd.btn = true;
